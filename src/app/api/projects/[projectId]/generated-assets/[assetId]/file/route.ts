...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const file = await getServices().assetService.openFile(user.id, projectId, assetId, variant);
    const range = parseRange(request.headers.get("range"), file.stream.byteSize);
    const streamed = range
      ? await (async () => {
          file.stream.stream.destroy();
          return getServices().assetService.openFile(user.id, projectId, assetId, variant, range);
        })()
      : file;
    ...
    if (!isAppError(error)) {
      logger.error("generated_asset.file_failed", {
