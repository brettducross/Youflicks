...
/** SHARE_LINK recipient ranged stream. Re-checks revoke. Never a public CDN. */
export async function GET(request: Request, context: RouteContext) {
  try {
    const { sessionId } = await context.params;
    const decodedSessionId = decodeURIComponent(sessionId);
    const file = await getServices().playbackService.openShareStream(decodedSessionId);
    const range = parseRange(request.headers.get("range"), file.stream.byteSize);
    const streamed = range
      ? await (async () => {
          file.stream.stream.destroy();
          return getServices().playbackService.openShareStream(decodedSessionId, range);
        })()
      : file;
    ...
    if (!isAppError(error)) {
      logger.error("publication.share_stream_failed", {
